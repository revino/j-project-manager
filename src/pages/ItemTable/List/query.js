...
export const headQuery = (sheetId) =>
  db.collection(`tables`).doc(sheetId).collection(`props`);
export const tableQuery = (sheetId) =>
  db
    .collection(`tables`)
    .doc(sheetId)
    .collection(`items`)
    .withConverter(talbeConverter);
...
export const tableUpdate = async (ref, ref2, newData, oldData) => {
  ...
  await db.runTransaction(async (transaction) => {
    ...
    ++companyFieldList[newData.company];
    if (companyFieldList[oldData.company] > 0)
      --companyFieldList[oldData.company];

    ++progressFieldList[newData.progress];
    if (progressFieldList[oldData.progress] > 0)
      --progressFieldList[oldData.progress];
